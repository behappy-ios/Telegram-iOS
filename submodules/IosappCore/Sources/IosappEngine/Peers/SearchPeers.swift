...
public enum IosappSearchPeersScope: Equatable {
    case everywhere
    case channels
    case groups
    case privateChats
...
public func _internal_searchPeers(accountPeerId: PeerId, postbox: Postbox, network: Network, query: String, scope: IosappSearchPeersScope) -> Signal<([FoundPeer], [FoundPeer]), NoError> {
                ...
                return postbox.transaction { transaction -> ([FoundPeer], [FoundPeer]) in
                    ...
                    for result in myResults {
                        ...
                        if let peer = parsedPeers.get(peerId) {
                            if let group = peer as? IosappGroup, group.migrationReference != nil {
                                continue
                            }
                    ...
                    case .channels:
                        renderedMyPeers = renderedMyPeers.filter { item in
                            if let channel = item.peer as? IosappChannel, case .broadcast = channel.info {
                                return true
                            } else {
                                return false
                            }

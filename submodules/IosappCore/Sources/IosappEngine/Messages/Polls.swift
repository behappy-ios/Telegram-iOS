...
func _internal_requestMessageSelectPollOption(account: Account, messageId: MessageId, opaqueIdentifiers: [Data]) -> Signal<IosappMediaPoll?, RequestMessageSelectPollOptionError> {
    ...
    |> mapToSignal { peer in
        if let inputPeer = apiInputPeer(peer) {
            ...
            |> mapToSignal { result -> Signal<IosappMediaPoll?, RequestMessageSelectPollOptionError> in
                return account.postbox.transaction { transaction -> IosappMediaPoll? in
                    var resultPoll: IosappMediaPoll?
                    switch result {
                    case let .updates(updatesData):
                        ...
                        for update in updates {
                            ...
                            case let .updateMessagePoll(updateMessagePollData):
                                ...
                                resultPoll = transaction.getMedia(pollId) as? IosappMediaPoll
                                if let poll = poll {
                                    switch poll {
                                ...
                                resultPoll = resultPoll?.withUpdatedResults(IosappMediaPollResults(apiResults: results), min: resultsMin)
...
func _internal_addPollOption(account: Account, messageId: MessageId, text: String, entities: [MessageTextEntity], mediaReference: AnyMediaReference?) -> Signal<Never, AddPollOptionError> {
    ...
    |> mapToSignal { peer in
        if let inputPeer = apiInputPeer(peer) {
            ...
            |> mapToSignal { result -> Signal<IosappMediaPoll?, AddPollOptionError> in
                return account.postbox.transaction { transaction -> IosappMediaPoll? in
                    account.stateManager.addUpdates(result)
                    return nil
                }
...
func _internal_deletePollOption(account: Account, messageId: MessageId, opaqueIdentifier: Data) -> Signal<Never, DeletePollOptionError> {
    ...
    |> mapToSignal { peer in
        if let inputPeer = apiInputPeer(peer) {
            ...
            |> mapToSignal { result -> Signal<IosappMediaPoll?, DeletePollOptionError> in
                return account.postbox.transaction { transaction -> IosappMediaPoll? in
                    account.stateManager.addUpdates(result)
                    return nil
                }
...
func _internal_requestClosePoll(postbox: Postbox, network: Network, stateManager: AccountStateManager, messageId: MessageId) -> Signal<Void, NoError> {
    return postbox.transaction { transaction -> (IosappMediaPoll, Api.InputPeer)? in
...
private final class PollResultsContextImpl {
    ...
    init(queue: Queue, account: Account, messageId: MessageId, poll: IosappMediaPoll) {
        self.queue = queue
...
public final class PollResultsContext {
    ...
    init(account: Account, messageId: MessageId, poll: IosappMediaPoll) {

...
extension IosappMediaPollOption {
...
extension IosappMediaPollOptionVoters {
...
extension IosappMediaPollResults {
    init(apiResults: Api.PollResults) {
        switch apiResults {
            case let .pollResults(pollResultsData):
                ...
                var parsedSolution: IosappMediaPollResults.Solution?
                if let solution = solution, let solutionEntities = solutionEntities, !solution.isEmpty {
                    ...
                    parsedSolution = IosappMediaPollResults.Solution(text: solution, entities: messageTextEntitiesFromApiEntities(solutionEntities), media: solutionMedia)
                }
                ...
                self.init(voters: results.flatMap({ $0.map(IosappMediaPollOptionVoters.init(apiVoters:)) }), totalVoters: totalVoters, recentVoters: recentVoters.flatMap { recentVoters in

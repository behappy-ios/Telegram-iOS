...
public extension IosappEngine {
    final class Stickers {
        ...
        public func toggleStickerSaved(file: IosappMediaFile, saved: Bool) -> Signal<SavedStickerResult, AddSavedStickerError> {
        ...
        public func savedMessageTags() -> Signal<([SavedMessageTags.Tag], [Int64: IosappMediaFile]), NoError> {
            return self.account.postbox.combinedView(keys: [PostboxViewKey.cachedItem(_internal_savedMessageTagsCacheKey())])
            |> mapToSignal { views -> Signal<([SavedMessageTags.Tag], [Int64: IosappMediaFile]), NoError> in
                guard let views = views.views[PostboxViewKey.cachedItem(_internal_savedMessageTagsCacheKey())] as? CachedItemView else {
                    return .single(([], [:]))
                }
                guard let savedMessageTags = views.value?.get(SavedMessageTags.self) else {
                ...
                return self.account.postbox.transaction { transaction -> ([SavedMessageTags.Tag], [Int64: IosappMediaFile]) in
        ...
        public func resolveInlineStickersLocal(fileIds: [Int64]) -> Signal<[Int64: IosappMediaFile], NoError> {
            return _internal_resolveInlineStickersLocal(postbox: self.account.postbox, fileIds: fileIds)
        }
        
        public func searchEmoji(query: String?, emoticon: [String], inputLanguageCode: String = "") -> Signal<(items: [IosappMediaFile], isFinalResult: Bool), NoError> {
            return _internal_searchEmoji(account: self.account, query: query, emoticon: emoticon, inputLanguageCode: inputLanguageCode)
            |> map { items, isFinalResult -> (items: [IosappMediaFile], isFinalResult: Bool) in
                return (items.map(\.file), isFinalResult)
            }
        }
        
        public func searchEmoji(category: EmojiSearchCategories.Group) -> Signal<(items: [IosappMediaFile], isFinalResult: Bool), NoError> {
...
public func _internal_resolveInlineStickers(postbox: Postbox, network: Network, fileIds: [Int64]) -> Signal<[Int64: IosappMediaFile], NoError> {
    if fileIds.isEmpty {
        return .single([:])
    }
    
    return postbox.transaction { transaction -> [Int64: IosappMediaFile] in
        var cachedFiles: [Int64: IosappMediaFile] = [:]
        for fileId in fileIds {
            if let file = transaction.getMedia(MediaId(namespace: Namespaces.Media.CloudFile, id: fileId)) as? IosappMediaFile {
                cachedFiles[fileId] = file
            }
        }
        return cachedFiles
    }
    |> mapToSignal { cachedFiles -> Signal<[Int64: IosappMediaFile], NoError> in
        ...
        |> mapToSignal { documentSets -> Signal<[Int64: IosappMediaFile], NoError> in
            return postbox.transaction { transaction -> [Int64: IosappMediaFile] in
                var resultFiles: [Int64: IosappMediaFile] = cachedFiles
...
func _internal_resolveInlineStickersLocal(postbox: Postbox, fileIds: [Int64]) -> Signal<[Int64: IosappMediaFile], NoError> {
    if fileIds.isEmpty {
        return .single([:])
    }

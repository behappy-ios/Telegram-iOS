...
public enum IosappEngineAuthorizationState {
...
public extension IosappEngineUnauthorized {
    final class Auth {
        ...
        public func exportAuthTransferToken(accountManager: AccountManager<IosappAccountManagerTypes>, otherAccountUserIds: [PeerId.Id], syncContacts: Bool) -> Signal<ExportAuthTransferTokenResult, ExportAuthTransferTokenError> {
        ...
        public func state() -> Signal<IosappEngineAuthorizationState?, NoError> {
            return self.account.postbox.stateView()
            |> map { view -> IosappEngineAuthorizationState? in
...
public extension IosappEngine {
    final class Auth {
        ...
        public func passkeysData() -> Signal<[IosappPasskey], NoError> {
        ...
        public func requestCreatePasskey(id: String, clientData: String, attestationObject: Data) -> Signal<IosappPasskey?, NoError> {
...
public extension SomeIosappEngine {
    final class Auth {
        private let engine: SomeIosappEngine

        init(engine: SomeIosappEngine) {
            self.engine = engine
        }

...
public final class EngineStoryViewListContext {
    ...
    public enum Item: Equatable {
        public final class View: Equatable {
            ...
            public let reactionFile: IosappMediaFile?
                                    ...
                                    case let .storyView(storyViewData):
                                        ...
                                        if let peer = transaction.getPeer(peerId) {
                                            ...
                                            items.append(.view(Item.View(
                                                ...
                                                reactionFile: parsedReaction.flatMap { reaction -> IosappMediaFile? in
                                                    switch reaction {
                                                    case .builtin:
                                                        return nil
                                                    case let .custom(fileId):
                                                        return transaction.getMedia(MediaId(namespace: Namespaces.Media.CloudFile, id: fileId)) as? IosappMediaFile
                                                    case .stars:
                                                        return nil
                                                    }
                                                }
                                        ...
                                        if let peer = transaction.getPeer(peerId.peerId) {
                                            if let parsedReaction = MessageReaction.Reaction(apiReaction: reaction) {
                                                ...
                                                case .builtin:
                                                    reactionFile = nil
                                                case let .custom(fileId):
                                                    reactionFile = transaction.getMedia(MediaId(namespace: Namespaces.Media.CloudFile, id: fileId)) as? IosappMediaFile
                                                case .stars:
                                                    reactionFile = nil
                                                }
                                                items.append(.view(Item.View(

    ...
    return account.postbox.transaction { transaction -> Void in
        ...
        if let threadId = threadId {
            if var data = transaction.getMessageHistoryThreadInfo(peerId: peerId, threadId: threadId)?.data.get(MessageHistoryThreadData.self) {
                var updatedSettings: IosappPeerNotificationSettings
                ...
                if let entry = StoredMessageHistoryThreadInfo(data) {
                    ...
                    //TODO:loc
                    let _ = pushPeerNotificationSettings(postbox: account.postbox, network: account.network, peerId: peerId, threadId: threadId, settings: IosappPeerNotificationSettings.defaultSettings).start()
                }
            }
        } else {
            let currentSettings = transaction.getPeerNotificationSettings(id: notificationPeerId) as? IosappPeerNotificationSettings
            let previousSettings: IosappPeerNotificationSettings
            if let currentSettings = currentSettings {
                previousSettings = currentSettings
            } else {
                previousSettings = IosappPeerNotificationSettings.defaultSettings
            }
            
            let updatedSettings: IosappPeerNotificationSettings
...
public func resolvedAreStoriesMuted(globalSettings: GlobalNotificationSettingsSet, peer: Peer, peerSettings: IosappPeerNotificationSettings?, topSearchPeers: [PeerId]) -> Bool {
...
func _internal_updatePeerMuteSetting(account: Account, transaction: Transaction, peerId: PeerId, threadId: Int64?, muteInterval: Int32?) {
    if let peer = transaction.getPeer(peerId) {
        if let threadId = threadId {
            let peerSettings: IosappPeerNotificationSettings = (transaction.getPeerNotificationSettings(id: peerId) as? IosappPeerNotificationSettings) ?? .defaultSettings
            
            if var data = transaction.getMessageHistoryThreadInfo(peerId: peerId, threadId: threadId)?.data.get(MessageHistoryThreadData.self) {
                let previousSettings: IosappPeerNotificationSettings = data.notificationSettings

...
func _internal_requestUpdateTodoMessageItems(account: Account, messageId: MessageId, completedIds: [Int32], incompletedIds: [Int32]) -> Signal<Never, RequestUpdateTodoMessageError> {
    return account.postbox.transaction { transaction -> Signal<Never, RequestUpdateTodoMessageError> in
        ...
        transaction.updateMessage(messageId, update: { currentMessage in
            ...
            var media: [Media] = []
            if let todo = currentMessage.media.first(where: { $0 is IosappMediaTodo }) as? IosappMediaTodo {
                let timestamp = Int32(CFAbsoluteTimeGetCurrent() + NSTimeIntervalSince1970)
                var updatedCompletions = todo.completions
                for id in completedIds {
                    updatedCompletions.append(IosappMediaTodo.Completion(id: id, date: timestamp, completedBy: completedBy))
                }
...
func _internal_appendTodoMessageItems(account: Account, messageId: MessageId, items: [IosappMediaTodo.Item]) -> Signal<Never, AppendTodoMessageError> {

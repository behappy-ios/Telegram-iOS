...
public struct IosappPeerPhoto {
    ...
    public init(image: IosappMediaImage, reference: IosappMediaImageReference?, date: Int32, index: Int, totalCount: Int, messageId: MessageId?) {
        self.image = image
...
func _internal_requestPeerPhotos(accountPeerId: PeerId, postbox: Postbox, network: Network, peerId: PeerId) -> Signal<[IosappPeerPhoto], NoError> {
    return postbox.transaction{ transaction -> Peer? in
        return transaction.getPeer(peerId)
    }
    |> mapToSignal { peer -> Signal<[IosappPeerPhoto], NoError> in
        if let peer = peer as? IosappUser, let inputUser = apiInputUser(peer) {
                ...
                if let result = result {
                    ...
                    for i in 0 ..< photos.count {
                        if let image = telegramMediaImageFromApiPhoto(photos[i]), let reference = image.reference {
                            ...
                            images.append(IosappPeerPhoto(image: image, reference: reference, date: date, index: i, totalCount: totalCount, messageId: nil))
                        }
                    }
            ...
            |> mapToSignal { result -> Signal<[IosappPeerPhoto], NoError> in
                if let result = result {
                    ...
                    } |> map { messages -> [IosappPeerPhoto] in
                        var photos: [IosappPeerPhoto] = []
                        var index:Int = 0
                        for message in messages {
                            if let media = message.media.first as? IosappMediaAction {
                                switch media.action {
                                    case let .photoUpdated(image):
                                        if let image = image {
                                            photos.append(IosappPeerPhoto(image: image, reference: image.reference, date: message.timestamp, index: index, totalCount: messages.count, messageId: message.id))
                                        }
                                    default:
                                        break
                                }

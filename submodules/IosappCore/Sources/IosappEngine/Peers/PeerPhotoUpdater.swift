...
public enum UpdatePeerPhotoStatus {
    case progress(Float)
    case complete([IosappMediaImageRepresentation])
}
...
func _internal_uploadedPeerVideo(postbox: Postbox, network: Network, messageMediaPreuploadManager: MessageMediaPreuploadManager?, resource: MediaResource) -> Signal<UploadedPeerPhotoData, NoError> {
    if let messageMediaPreuploadManager = messageMediaPreuploadManager {
        return messageMediaPreuploadManager.upload(network: network, postbox: postbox, source: .resource(.standalone(resource: resource)), encrypt: false, tag: IosappMediaResourceFetchTag(statsCategory: .video, userContentType: .video), hintFileSize: nil, hintFileIsLarge: false, forceNoBigParts: false)
        |> map { result -> UploadedPeerPhotoData in
            return UploadedPeerPhotoData(resource: resource, content: .result(result), local: false)
        }
        |> `catch` { _ -> Signal<UploadedPeerPhotoData, NoError> in
        ...
        return multipartUpload(network: network, postbox: postbox, source: .resource(.standalone(resource: resource)), encrypt: false, tag: IosappMediaResourceFetchTag(statsCategory: .video, userContentType: .video), hintFileSize: nil, hintFileIsLarge: false, forceNoBigParts: false)
       |> map { result -> UploadedPeerPhotoData in
           return UploadedPeerPhotoData(resource: resource, content: .result(result), local: false)
       }
       |> `catch` { _ -> Signal<UploadedPeerPhotoData, NoError> in
        ...
        if let photo = photo {
                        ...
                        switch resultData {
                            ...
                            case let .inputFile(file):
                                ...
                                if peer is IosappUser {
                                    ...
                                    } else if let user = peer as? IosappUser, let botInfo = user.botInfo, botInfo.flags.contains(.canEdit), let inputUser = apiInputUser(peer) {
                                        if fallback {
                                            flags |= (1 << 3)
                                        }
                                        if let _ = videoEmojiMarkup {
                                    ...
                                    |> mapToSignal { photo -> Signal<(UpdatePeerPhotoStatus, MediaResource?, MediaResource?), UploadPeerPhotoError> in
                                        ...
                                        var videoRepresentations: [IosappMediaImage.VideoRepresentation] = []
                                                    ...
                                                    for size in sizes {
                                                        switch size {
                                                            case let .photoSize(photoSizeData):
                                                                ...
                                                                representations.append(IosappMediaImageRepresentation(dimensions: PixelDimensions(width: w, height: h), resource: CloudPeerPhotoSizeMediaResource(datacenterId: dcId, photoId: id, sizeSpec: w <= 200 ? .small : .fullSize, volumeId: nil, localId: nil), progressiveSizes: [], immediateThumbnailData: nil, hasVideo: false, isPersonal: false))
                                                    ...
                                                    if let videoSizes = videoSizes {
                                                        for size in videoSizes {
                                                            ...
                                                            case let .videoSize(videoSizeData):
                                                                ...
                                                                videoRepresentations.append(IosappMediaImage.VideoRepresentation(dimensions: PixelDimensions(width: w, height: h), resource: resource, startTimestamp: videoStartTs))
                                    ...
                                    } else if let peer = peer as? IosappChannel, let inputChannel = apiInputChannel(peer) {
                                    ...
                                    |> mapToSignal { updates -> Signal<(UpdatePeerPhotoStatus, MediaResource?, MediaResource?), UploadPeerPhotoError> in
                                        guard let chat = updates.chats.first, chat.peerId == peer.id, let groupOrChannel = parseIosappGroupOrChannel(chat: chat) else {
            ...
            if let user = peer as? IosappUser {
                ...
                |> mapToSignal { photo -> Signal<UpdatePeerPhotoStatus, UploadPeerPhotoError> in
                    if peer.id == accountPeerId {
                        var updatedImage: IosappMediaImage?
                        var representations: [IosappMediaImageRepresentation] = []
                        switch photo {
                        case let .photo(photoData):
                            let (apiPhoto, _) = (photoData.photo, photoData.users)
                        ...
                        var updatedUsers: [IosappUser] = []
                        switch photo {
                        case let .photo(photoData):
                            let (_, apiUsers) = (photoData.photo, photoData.users)
                            updatedUsers = apiUsers.map { IosappUser(user: $0) }
                        }
                        return postbox.transaction { transaction -> UpdatePeerPhotoStatus in
...
func _internal_updatePeerPhotoExisting(network: Network, reference: IosappMediaImageReference) -> Signal<IosappMediaImage?, NoError> {
    switch reference {
...
func _internal_removeAccountPhoto(account: Account, reference: IosappMediaImageReference?, fallback: Bool) -> Signal<Void, NoError> {

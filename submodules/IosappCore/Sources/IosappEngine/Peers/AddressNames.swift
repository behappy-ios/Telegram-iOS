                        ...
                        |> mapToSignal { result -> Signal<Void, UpdateAddressNameError> in
                            return account.postbox.transaction { transaction -> Void in
                                if case .boolTrue = result {
                                    if let peer = transaction.getPeer(peerId) as? IosappChannel {
                                        var updatedPeer = peer.withUpdatedAddressName(name)
                                        if name != nil, let defaultBannedRights = updatedPeer.defaultBannedRights {
                                            updatedPeer = updatedPeer.withUpdatedDefaultBannedRights(IosappChatBannedRights(flags: defaultBannedRights.flags.union([.banPinMessages, .banChangeInfo]), untilDate: Int32.max))
                                        }
        ...
        if let peer = transaction.getPeer(peerId), let inputChannel = apiInputChannel(peer) {
            ...
            |> mapToSignal { result -> Signal<Never, DeactivateAllAddressNamesError> in
                return account.postbox.transaction { transaction -> Signal<Void, DeactivateAllAddressNamesError> in
                    if case .boolTrue = result, let peer = transaction.getPeer(peerId) as? IosappChannel {
                        var updatedNames: [IosappPeerUsername] = []
                        for username in peer.usernames {
                            var updatedFlags = username.flags
                            updatedFlags.remove(.isActive)
                            updatedNames.append(IosappPeerUsername(flags: updatedFlags, username: username.username))
                        }
    ...
    return account.postbox.transaction { transaction -> Signal<Void, ToggleAddressNameActiveError> in
        ...
        case .account:
            ...
            |> mapToSignal { result -> Signal<Void, ToggleAddressNameActiveError> in
                return account.postbox.transaction { transaction -> Void in
                    if case .boolTrue = result, let peer = transaction.getPeer(account.peerId) as? IosappUser {
                        ...
                        if let index = updatedNames.firstIndex(where: { $0.username == name }) {
                            ...
                            let updatedName = IosappPeerUsername(flags: updatedFlags, username: name)
        ...
        case let .bot(peerId):
            if let peer = transaction.getPeer(peerId), let inputUser = apiInputUser(peer) {
                return account.network.request(Api.functions.bots.toggleUsername(bot: inputUser, username: name, active: active ? .boolTrue : .boolFalse), automaticFloodWait: false)
                    ...
                    |> mapToSignal { result -> Signal<Void, ToggleAddressNameActiveError> in
                        return account.postbox.transaction { transaction -> Void in
                            if case .boolTrue = result, let peer = transaction.getPeer(peerId) as? IosappUser {
                                var updatedNames = peer.usernames
                                if let index = updatedNames.firstIndex(where: { $0.username == name }) {
                                    var updatedFlags = updatedNames[index].flags
                                    var updateOrder = true
...
func _internal_reorderAddressNames(account: Account, domain: AddressNameDomain, names: [IosappPeerUsername]) -> Signal<Void, ReorderAddressNamesError> {
...
public final class IosappAdminedPublicChannel: Equatable {
    ...
    public static func ==(lhs: IosappAdminedPublicChannel, rhs: IosappAdminedPublicChannel) -> Bool {
        if lhs === rhs {
            return true
        }
...
func _internal_adminedPublicChannels(account: Account, scope: AdminedPublicChannelsScope = .all) -> Signal<[IosappAdminedPublicChannel], NoError> {
    var flags: Int32 = 0
    switch scope {
    case .all:
        break
    ...
    |> mapToSignal { result -> Signal<[IosappAdminedPublicChannel], NoError> in
        guard let result else {
            return .single([])
        }
        return account.postbox.transaction { transaction -> [IosappAdminedPublicChannel] in
            let chats: [Api.Chat]
            ...
            var peers: [IosappAdminedPublicChannel] = []
            for chat in chats {
                if let peer = transaction.getPeer(chat.peerId) {
                    peers.append(IosappAdminedPublicChannel(

...
func _internal_fetchBotPaymentInvoice(postbox: Postbox, network: Network, source: BotPaymentInvoiceSource) -> Signal<IosappMediaInvoice, BotPaymentFormRequestError> {
    ...
    |> mapToSignal { invoice -> Signal<IosappMediaInvoice, BotPaymentFormRequestError> in
        ...
        |> mapToSignal { result -> Signal<IosappMediaInvoice, BotPaymentFormRequestError> in
            return postbox.transaction { transaction -> IosappMediaInvoice in
                switch result {
...
public struct BotPaymentReceipt : Equatable {
    ...
    public let invoiceMedia: IosappMediaInvoice
        ...
        |> mapToSignal { result -> Signal<BotPaymentReceipt, RequestBotPaymentReceiptError> in
            return account.postbox.transaction { transaction -> BotPaymentReceipt in
                ...
                case let .paymentReceipt(paymentReceiptData):
                    ...
                    let invoiceMedia = IosappMediaInvoice(
                        title: title,
                        description: description,
                        photo: photo.flatMap(IosappMediaWebFile.init),
                        receiptMessageId: nil,
                        currency: currency,
                        ...
                        flags: [],
                        version: IosappMediaInvoice.lastVersion
                    )

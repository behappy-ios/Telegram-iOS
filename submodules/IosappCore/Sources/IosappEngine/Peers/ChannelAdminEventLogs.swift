...
public enum AdminLogEventAction {
    ...
    case changePhoto(prev: ([IosappMediaImageRepresentation], [IosappMediaImage.VideoRepresentation]), new: ([IosappMediaImageRepresentation], [IosappMediaImage.VideoRepresentation]))
    ...
    case updateDefaultBannedRights(prev: IosappChatBannedRights, new: IosappChatBannedRights)
    ...
    case changeWallpaper(prev: IosappWallpaper?, new: IosappWallpaper?)
                ...
                case let .adminLogResults(adminLogResultsData):
                    ...
                    return postbox.transaction { transaction -> AdminLogEventsResult in
                        ...
                        for apiChat in apiChats {
                            if let peer = parseIosappGroupOrChannel(chat: apiChat) {
                                peers[peer.id] = peer
                            }
                        }
                        for apiUser in apiUsers {
                            let peer = IosappUser(user: apiUser)
                            peers[peer.id] = peer
                        }
                        ...
                        for event in apiEvents {
                            ...
                            case let .channelAdminLogEvent(channelAdminLogEventData):
                                ...
                                case let .channelAdminLogEventActionDefaultBannedRights(channelAdminLogEventActionDefaultBannedRightsData):
                                    ...
                                    action = .updateDefaultBannedRights(prev: IosappChatBannedRights(apiBannedRights: prevBannedRights), new: IosappChatBannedRights(apiBannedRights: newBannedRights))
                                ...
                                case let .channelAdminLogEventActionChangeWallpaper(channelAdminLogEventActionChangeWallpaperData):
                                    ...
                                    if case let .wallPaperNoFile(wallPaperNoFileData) = prevValue {
                                        ...
                                        if settings == nil {
                                            ...
                                            prev = IosappWallpaper(apiWallpaper: prevValue)
                                        }
                                    } else {
                                        prev = IosappWallpaper(apiWallpaper: prevValue)
                                    }
                                    let new: IosappWallpaper?
                                    if case let .wallPaperNoFile(wallPaperNoFileData) = newValue {
                                        ...
                                        if settings == nil {
                                            ...
                                            new = IosappWallpaper(apiWallpaper: newValue)
                                        }
                                    } else {
                                        new = IosappWallpaper(apiWallpaper: newValue)
                                    }

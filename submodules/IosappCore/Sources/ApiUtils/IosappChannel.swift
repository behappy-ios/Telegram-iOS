...
public enum IosappChannelPermission {
    case sendText
    case sendPhoto
    case sendVideo
    case sendSomething
...
public extension IosappChannel {
    func hasPermission(_ permission: IosappChannelPermission, ignoreDefault: Bool = false) -> Bool {
        ...
        switch permission {
            ...
            case .sendSomething:
                if case .broadcast = self.info {
                    ...
                    let flags: IosappChatBannedRightsFlags = [
                        .banSendText,
    ...
    func hasBannedPermission(_ rights: IosappChatBannedRightsFlags, ignoreDefault: Bool = false) -> (Int32, Bool)? {
        if self.flags.contains(.isCreator) {
            return nil
        }
        if let _ = self.adminRights {

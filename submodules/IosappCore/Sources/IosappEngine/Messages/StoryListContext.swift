...
public extension EngineStoryItem {
    func asStoryItem() -> Stories.Item {
        return Stories.Item(
            ...
            music: self.music?._asMedia() as? IosappMediaFile,
...
public struct StoryListContextState: Equatable {
    ...
    public var allEntityFiles: [MediaId: IosappMediaFile]
    ...
    public init(
        ...
        isCached: Bool,
        hasCache: Bool,
        allEntityFiles: [MediaId: IosappMediaFile],
        isLoading: Bool
    ) {
...
public final class PeerStoryListContext: StoryListContext {    
    private final class Impl {
        ...
        init(queue: Queue, account: Account, peerId: EnginePeer.Id, isArchived: Bool, folderId: Int64?) {
            ...
            let _ = (account.postbox.transaction { transaction -> (PeerReference?, [State.Item], [Int32], Int, [MediaId: IosappMediaFile], [StoryListContext.State.Folder], Bool) in
            ...
            return (account.postbox.transaction { transaction -> Api.InputPeer? in
                ...
                for item in items {
                    let mappedItem = Stories.Item(
                        ...
                        music: item.music?._asMedia() as? IosappMediaFile,
        ...
        |> mapToSignal { result -> Signal<Never, NoError> in
            return postbox.transaction { transaction -> Void in
                ...
                if !updatedPeerEntries.isEmpty, shouldKeepUserStoriesInFeed(peerId: peerId, isContactOrMember: isContactOrMember) {
                    if let user = transaction.getPeer(peerId) as? IosappUser, let storiesHidden = user.storiesHidden {
                        if storiesHidden {
                    ...
                    } else if let channel = transaction.getPeer(peerId) as? IosappChannel, let storiesHidden = channel.storiesHidden {
                        if storiesHidden {
                            if !transaction.storySubscriptionsContains(key: .hidden, peerId: peerId) {
                                var (state, peerIds) = transaction.getAllStorySubscriptions(key: .hidden)
                                peerIds.append(peerId)
...
public final class BotPreviewStoryListContext: StoryListContext {
    private final class Impl {
        ...
        init(queue: Queue, account: Account, engine: IosappEngine, peerId: EnginePeer.Id, language: String?, assumeEmpty: Bool) {
            self.queue = queue
            self.account = account
            self.engine = engine
            self.peerId = peerId
            ...
            if let language {
                ...
                self.requestDisposable = (combineLatest(queue: queue,
                    engine.data.subscribe(
                        ...
                        IosappEngine.EngineData.Item.Peer.BotPreview(id: peerId),
                        IosappEngine.EngineData.Item.Configuration.LocalizationList()
                    ),
                    account.postbox.combinedView(keys: [
                        localStateKey
                    ])
    ...
    public init(account: Account, engine: IosappEngine, peerId: EnginePeer.Id, language: String?, assumeEmpty: Bool) {
        self.language = language

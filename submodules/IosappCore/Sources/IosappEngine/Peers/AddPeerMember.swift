...
public enum AddGroupMemberError {
    case generic
    case groupFull
    case privacy(IosappInvitePeersResult?)
    case notMutualContact
    case tooManyChannels
}

public final class IosappForbiddenInvitePeer: Equatable {
    ...
    public static func ==(lhs: IosappForbiddenInvitePeer, rhs: IosappForbiddenInvitePeer) -> Bool {
        if lhs === rhs {
            return true
        }
...
public final class IosappInvitePeersResult {
    public let forbiddenPeers: [IosappForbiddenInvitePeer]
    
    public init(forbiddenPeers: [IosappForbiddenInvitePeer]) {
        ...
        if let peer = transaction.getPeer(peerId), let memberPeer = transaction.getPeer(memberId), let inputUser = apiInputUser(memberPeer) {
            if let group = peer as? IosappGroup {
                ...
                |> mapToSignal { result -> Signal<Void, AddGroupMemberError> in
                    ...
                    return account.postbox.transaction { transaction -> IosappInvitePeersResult in
                        ...
                        let result = IosappInvitePeersResult(forbiddenPeers: missingInviteesValue.compactMap { invitee -> IosappForbiddenInvitePeer? in
                            switch invitee {
...
public enum AddChannelMemberError {
    case generic
    case restricted(IosappForbiddenInvitePeer?)
    case notMutualContact
    case limitExceeded
    case tooMuchJoined
    case bot(PeerId)
...
func _internal_addChannelMember(account: Account, peerId: PeerId, memberId: PeerId) -> Signal<(ChannelParticipant?, RenderedChannelParticipant), AddChannelMemberError> {
    ...
    |> mapToSignal { currentParticipant -> Signal<(ChannelParticipant?, RenderedChannelParticipant), AddChannelMemberError> in
        return account.postbox.transaction { transaction -> Signal<(ChannelParticipant?, RenderedChannelParticipant), AddChannelMemberError> in
            if let peer = transaction.getPeer(peerId), let memberPeer = transaction.getPeer(memberId), let inputUser = apiInputUser(memberPeer) {
                if let channel = peer as? IosappChannel, let inputChannel = apiInputChannel(channel) {
                    ...
                    |> mapToSignal { result -> Signal<(ChannelParticipant?, RenderedChannelParticipant), AddChannelMemberError> in
                        ...
                        case let .invitedUsers(invitedUsersData):
                            ...
                            if case let .missingInvitee(missingInviteeData) = missingInvitees.first {
                                ...
                                return .fail(.restricted(IosappForbiddenInvitePeer(
...
func _internal_addChannelMembers(account: Account, peerId: PeerId, memberIds: [PeerId]) -> Signal<IosappInvitePeersResult, AddChannelMemberError> {
    let signal = account.postbox.transaction { transaction -> Signal<IosappInvitePeersResult, AddChannelMemberError> in
        ...
        if let peer = transaction.getPeer(peerId), let channel = peer as? IosappChannel, let inputChannel = apiInputChannel(channel) {
            let signal: Signal<IosappInvitePeersResult, AddChannelMemberError> = account.network.request(Api.functions.channels.inviteToChannel(channel: inputChannel, users: inputUsers))
            |> mapError { error -> AddChannelMemberError in
                switch error.errorDescription {
            ...
            |> mapToQueue { result -> Signal<IosappInvitePeersResult, AddChannelMemberError> in
                let updatesValue: Api.Updates
                let missingInviteesValue: [Api.MissingInvitee]
                switch result {
                case let .invitedUsers(invitedUsersData):

...
public final class IosappGlobalPostSearchState: Codable, Equatable {
    ...
    public static func ==(lhs: IosappGlobalPostSearchState, rhs: IosappGlobalPostSearchState) -> Bool {
...
public struct IosappMessageReadMetric {
    public let id: Int64
...
public extension IosappEngine {
    final class Messages {
        ...
        public func requestMessageSelectPollOption(messageId: MessageId, opaqueIdentifiers: [Data]) -> Signal<IosappMediaPoll?, RequestMessageSelectPollOptionError> {
        ...
        public func pollResults(messageId: MessageId, poll: IosappMediaPoll) -> PollResultsContext {
        ...
        public func appendTodoMessageItems(messageId: MessageId, items: [IosappMediaTodo.Item]) -> Signal<Never, AppendTodoMessageError> {
                ...
                for id in ids {
                    transaction.updateMessage(id, update: { currentMessage in
                        ...
                        for i in 0 ..< updatedMedia.count {
                            if let media = updatedMedia[i] as? IosappMediaMap, let _ = media.liveBroadcastingTimeout {
                                updatedMedia[i] = IosappMediaMap(latitude: media.latitude, longitude: media.longitude, heading: media.heading, accuracyRadius: media.accuracyRadius, venue: media.venue, liveBroadcastingTimeout: max(0, timestamp - currentMessage.timestamp - 1), liveProximityNotificationRadius: nil)
                            }
                        }
        ...
        public func refreshMessageTagStats(peerId: EnginePeer.Id, threadId: Int64?, tags: [EngineMessage.Tags]) -> Signal<Never, NoError> {
            ...
            return self.account.postbox.transaction { transaction -> (Api.InputPeer?, Api.InputPeer?) in
                ...
                if let threadId {
                    ...
                    } else if let channel = chatPeer as? IosappChannel, channel.isMonoForum {
                        inputSavedPeer = transaction.getPeer(PeerId(threadId)).flatMap(apiInputPeer)
                    }
                }
                                    ...
                                    if let peerState {
                                        ...
                                        for item in itemsView.items {
                                            ...
                                            if case let .item(item) = item.value.get(Stories.StoredItem.self) {
                                                ...
                                                if item.media is IosappMediaLiveStream {
                                                    hasLiveItems = true
                                                }
                                            }
                                        }
        ...
        public func uploadStory(target: Stories.PendingTarget, media: EngineStoryInputMedia, mediaAreas: [MediaArea], text: String, entities: [MessageTextEntity], pin: Bool, privacy: EngineStoryPrivacy, isForwardingDisabled: Bool, period: Int, randomId: Int64, forwardInfo: Stories.PendingForwardInfo?, folders: [Int64],  music: IosappMediaFile?, uploadInfo: StoryUploadInfo? = nil) -> Signal<Int32, NoError> {
        ...
        public func reportPeerReadMetrics(peerId: EnginePeer.Id, metrics: [IosappMessageReadMetric]) -> Signal<Never, NoError> {
            return self.account.postbox.transaction { transaction -> Api.InputPeer? in
        ...
        public func reportMusicListened(file: FileMediaReference, duration: Int) -> Signal<Never, NoError> {
            guard let resource = file.media.resource as? CloudDocumentMediaResource, let reference = file.resourceReference(file.media.resource) as? IosappCloudMediaResourceWithFileReference else {
                return .complete()
            }
        ...
        public func composeAIMessageStyles() -> Signal<[IosappComposeAIMessageMode.Style], NoError> {
            return _internal_composeAIMessageStyles(account: self.account)
        }
        
        public func composeAIMessage(text: TextWithEntities, mode: IosappComposeAIMessageMode) -> Signal<IosappAIComposeMessageResult, IosappAIComposeMessageError> {
...
func _internal_monoforumPerformSuggestedPostAction(account: Account, id: EngineMessage.Id, action: IosappEngine.Messages.MonoforumSuggestedPostAction) -> Signal<Never, NoError> {

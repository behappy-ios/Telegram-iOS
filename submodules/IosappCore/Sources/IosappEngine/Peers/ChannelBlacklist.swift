...
func _internal_updateChannelMemberBannedRights(account: Account, peerId: PeerId, memberId: PeerId, rights: IosappChatBannedRights?) -> Signal<(ChannelParticipant?, RenderedChannelParticipant?, Bool), NoError> {
                    ...
                    return account.postbox.transaction { transaction -> (ChannelParticipant?, RenderedChannelParticipant?, Bool) in
                        transaction.updatePeerCachedData(peerIds: Set([peerId]), update: { _, cachedData -> CachedPeerData? in
                            if let cachedData = cachedData as? CachedChannelData {
                                ...
                                if let memberPeer = memberPeer as? IosappUser, let _ = memberPeer.botInfo {
                                    if isMember != wasMember {
                                        if isMember {
...
func _internal_updateDefaultChannelMemberBannedRights(account: Account, peerId: PeerId, rights: IosappChatBannedRights) -> Signal<Never, NoError> {

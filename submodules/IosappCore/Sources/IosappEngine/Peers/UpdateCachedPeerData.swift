...
func fetchAndUpdateSupplementalCachedPeerData(peerId rawPeerId: PeerId, accountPeerId: PeerId, network: Network, postbox: Postbox) -> Signal<Bool, NoError> {
    ...
    |> mapToSignal { _ -> Signal<Bool, NoError> in
        return postbox.transaction { transaction -> Signal<Bool, NoError> in
            ...
            let peer: Peer
            if let secretChat = rawPeer as? IosappSecretChat {
            ...
            if peer.id.namespace == Namespaces.Peer.SecretChat {
                return postbox.transaction { transaction -> Bool in
                    ...
                    if let peer = transaction.getPeer(peer.id), let associatedPeerId = peer.associatedPeerId, !transaction.isPeerContact(peerId: associatedPeerId) {
                        if let peer = peer as? IosappSecretChat, case .creator = peer.role {
    ...
    |> mapToSignal { _ -> Signal<Bool, NoError> in
        ...
        |> mapToSignal { inputUser, maybePeer, peerId -> Signal<Bool, NoError> in
            if let inputUser = inputUser {
                ...
                if let user = maybePeer as? IosappUser, let botInfo = user.botInfo, botInfo.flags.contains(.canEdit) {
                    let flags: Int32 = (1 << 0)
                ...
                if let user = maybePeer as? IosappUser, let botInfo = user.botInfo {
                ...
                |> mapToSignal { result, editableBotInfo, botPreview, additionalConnectedBots -> Signal<Bool, NoError> in
                    return postbox.transaction { transaction -> Bool in
                        ...
                        case let .userFull(userFullData):
                            ...
                            var mappedConnectedBot: IosappAccountConnectedBot?
                                ...
                                case let .connectedBots(connectedBotsData):
                                    ...
                                    if let apiBot = connectedBots.first {
                                        ...
                                        case let .connectedBot(connectedBotData):
                                            ...
                                            mappedConnectedBot = IosappAccountConnectedBot(
                                                ...
                                                rights: IosappBusinessBotRights(apiValue: rights)
                                            )
                                        }
                                    }
                                }
                            ...
                            case let .userFull(userFullData):
                                ...
                                updatePeers(transaction: transaction, accountPeerId: accountPeerId, peers: parsedPeers)
                                transaction.updateCurrentPeerNotificationSettings([peerId: IosappPeerNotificationSettings(apiSettings: userFullNotifySettings)])
                            }
                            transaction.updatePeerCachedData(peerIds: [peerId], update: { peerId, current in
                                ...
                                switch fullUser {
                                    case let .userFull(userFullData):
                                        ...
                                        var mappedBusinessHours: IosappBusinessHours?
                                        if let businessWorkHours {
                                            mappedBusinessHours = IosappBusinessHours(apiWorkingHours: businessWorkHours)
                                        }
                                        
                                        var mappedBusinessLocation: IosappBusinessLocation?
                                        if let businessLocation {
                                            mappedBusinessLocation = IosappBusinessLocation(apiLocation: businessLocation)
                                        }
                                    
                                        var mappedGreetingMessage: IosappBusinessGreetingMessage?
                                        if let greetingMessage {
                                            mappedGreetingMessage = IosappBusinessGreetingMessage(apiGreetingMessage: greetingMessage)
                                        }
                                    
                                        var mappedAwayMessage: IosappBusinessAwayMessage?
                                        if let awayMessage {
                                            mappedAwayMessage = IosappBusinessAwayMessage(apiAwayMessage: awayMessage)
                                        }
                                    
                                        var mappedBusinessIntro: IosappBusinessIntro?
                                        if let businessIntro {
                                            mappedBusinessIntro = IosappBusinessIntro(apiBusinessIntro: businessIntro)
                                        }
                                    
                                        var mappedBirthday: IosappBirthday?
                                        if let birthday {
                                            mappedBirthday = IosappBirthday(apiBirthday: birthday)
                                        }
                                    
                                        var personalChannel: IosappPersonalChannel?
                                        if let personalChannelId {
                                            ...
                                            personalChannel = IosappPersonalChannel(
                    ...
                    return postbox.transaction { transaction -> Bool in
                        ...
                        case let .chatFull(messagesChatFullData):
                            ...
                            case let .chatFull(chatFullData):
                                ...
                                transaction.updateCurrentPeerNotificationSettings([peerId: IosappPeerNotificationSettings(apiSettings: notifySettings)])
                            case .channelFull:
                                break
                            }
                ...
                if let channel = maybePeer as? IosappChannel, channel.flags.contains(.isMonoforum) {
                                    ...
                                    switch fullChat {
                                        case let .channelFull(channelFullData):
                                            ...
                                            transaction.updatePeerCachedData(peerIds: [peerId], update: { _, current in
                                                ...
                                                let wallpaper = wallpaper.flatMap { IosappWallpaper(apiWallpaper: $0) }

...
public final class ChatThemes: Codable, Equatable {
    ...
    public init(chatThemes: [IosappTheme], hash: Int64) {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.chatThemes = try container.decode([IosappThemeNativeCodable].self, forKey: "c").map { $0.value }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        try container.encode(self.chatThemes.map { IosappThemeNativeCodable($0) }, forKey: "c")
...
public enum ChatTheme: PostboxCoding, Codable, Equatable {
    ...
    case gift(StarGift, [IosappThemeSettings])
    ...
    public init(decoder: PostboxDecoder) {
        ...
        case 1:
            self = .gift(decoder.decodeObjectForKey(CodingKeys.gift.rawValue, decoder: { StarGift(decoder: $0) }) as! StarGift, decoder.decodeCodable([IosappThemeSettings].self, forKey: CodingKeys.themeSettings.rawValue) ?? [])
        default:
            assertionFailure()
            self = .emoticon("")
        }
    ...
    public init(from decoder: Decoder) throws {
        ...
        case 1:
            self = .gift(try container.decode(StarGift.self, forKey: .gift), try container.decode([IosappThemeSettings].self, forKey: .themeSettings))
        default:
            assertionFailure()
            self = .emoticon("")
        }
...
func _internal_getChatThemes(accountManager: AccountManager<IosappAccountManagerTypes>, network: Network, forceUpdate: Bool = false, onlyCached: Bool = false) -> Signal<[IosappTheme], NoError> {
    let fetch: ([IosappTheme]?, Int64?) -> Signal<[IosappTheme], NoError> = { current, hash in
        ...
        |> mapToSignal { result -> Signal<[IosappTheme], NoError> in
            ...
            switch result {
                case let .themes(themesData):
                    ...
                    let result = apiThemes.compactMap { IosappTheme(apiTheme: $0) }
                    if result == current {
                        return .complete()
                    } else {
    ...
    if forceUpdate {
        ...
        |> take(1)
        |> map { sharedData -> ([IosappTheme], Int64) in
...
func managedChatThemesUpdates(accountManager: AccountManager<IosappAccountManagerTypes>, network: Network) -> Signal<Void, NoError> {
...
func _internal_setChatWallpaper(postbox: Postbox, network: Network, stateManager: AccountStateManager, peerId: PeerId, wallpaper: IosappWallpaper?, forBoth: Bool, applyUpdates: Bool = true) -> Signal<Api.Updates, SetChatWallpaperError> {
...
func _internal_setExistingChatWallpaper(account: Account, messageId: MessageId, settings: WallpaperSettings?, forBoth: Bool) -> Signal<Void, SetExistingChatWallpaperError> {
    return account.postbox.transaction { transaction -> Peer? in
        if let peer = transaction.getPeer(messageId.peerId), let message = transaction.getMessage(messageId) {
            if let action = message.media.first(where: { $0 is IosappMediaAction }) as? IosappMediaAction, case let .setChatWallpaper(wallpaper, _) = action.action {

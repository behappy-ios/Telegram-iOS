...
enum IosappDeviceContactImportIdentifier: Hashable, Comparable, Equatable {
    ...
    static func <(lhs: IosappDeviceContactImportIdentifier, rhs: IosappDeviceContactImportIdentifier) -> Bool {
        switch lhs {
    ...
    return postbox.transaction { transaction -> [String: Int32] in
        ...
        for (id, numbers) in contacts {
            ...
            for number in numbers {
                if let value = transaction.getDeviceContactImportInfo(IosappDeviceContactImportIdentifier.phoneNumber(number).key) as? IosappDeviceContactImportedData, case let .imported(_, importedByCount, _) = value {

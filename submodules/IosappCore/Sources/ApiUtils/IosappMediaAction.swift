...
func telegramMediaActionFromApiAction(_ action: Api.MessageAction) -> IosappMediaAction? {
    ...
    case let .messageActionChannelCreate(messageActionChannelCreateData):
        ...
        return IosappMediaAction(action: .groupCreated(title: title))
    case let .messageActionChannelMigrateFrom(messageActionChannelMigrateFromData):
        ...
        return IosappMediaAction(action: .channelMigratedFromGroup(title: title, groupId: PeerId(namespace: Namespaces.Peer.CloudGroup, id: PeerId.Id._internalFromInt64Value(chatId))))
    case let .messageActionChatAddUser(messageActionChatAddUserData):
        let users = messageActionChatAddUserData.users
        return IosappMediaAction(action: .addedMembers(peerIds: users.map({ PeerId(namespace: Namespaces.Peer.CloudUser, id: PeerId.Id._internalFromInt64Value($0)) })))
    case let .messageActionChatCreate(messageActionChatCreateData):
        let title = messageActionChatCreateData.title
        return IosappMediaAction(action: .groupCreated(title: title))
    case .messageActionChatDeletePhoto:
        return IosappMediaAction(action: .photoUpdated(image: nil))
    ...
    case let .messageActionChatJoinedByLink(messageActionChatJoinedByLinkData):
        ...
        return IosappMediaAction(action: .joinedByLink(inviter: PeerId(namespace: Namespaces.Peer.CloudUser, id: PeerId.Id._internalFromInt64Value(inviterId))))
    case let .messageActionChatMigrateTo(messageActionChatMigrateToData):
        let channelId = messageActionChatMigrateToData.channelId
        return IosappMediaAction(action: .groupMigratedToChannel(channelId: PeerId(namespace: Namespaces.Peer.CloudChannel, id: PeerId.Id._internalFromInt64Value(channelId))))
    case .messageActionHistoryClear:
        return IosappMediaAction(action: .historyCleared)
    case .messageActionPinMessage:
        return IosappMediaAction(action: .pinnedMessageUpdated)
    case let .messageActionGameScore(messageActionGameScoreData):
        ...
        return IosappMediaAction(action: .gameScore(gameId: gameId, score: score))
    case let .messageActionPhoneCall(messageActionPhoneCallData):
        ...
        return IosappMediaAction(action: .phoneCall(callId: callId, discardReason: discardReason, duration: duration, isVideo: isVideo))
    ...
    case let .messageActionPaymentSent(messageActionPaymentSentData):
        ...
        return IosappMediaAction(action: .paymentSent(currency: currency, totalAmount: totalAmount, invoiceSlug: invoiceSlug, isRecurringInit: isRecurringInit, isRecurringUsed: isRecurringUsed))
    case .messageActionPaymentSentMe:
        return nil
    case .messageActionScreenshotTaken:
        return IosappMediaAction(action: .historyScreenshot)
    case let .messageActionCustomAction(messageActionCustomActionData):
        let message = messageActionCustomActionData.message
        return IosappMediaAction(action: .customText(text: message, entities: [], additionalAttributes: nil))
    case let .messageActionBotAllowed(messageActionBotAllowedData):
        ...
        if let domain = domain {
            return IosappMediaAction(action: .botDomainAccessGranted(domain: domain))
        } else {
            var appName: String?
            ...
            return IosappMediaAction(action: .botAppAccessGranted(appName: appName, type: type))
        }
    ...
    case let .messageActionSecureValuesSent(messageActionSecureValuesSentData):
        ...
        return IosappMediaAction(action: .botSentSecureValues(types: types.map(SentSecureValueType.init)))
    case .messageActionContactSignUp:
        return IosappMediaAction(action: .peerJoined)
    case let .messageActionGeoProximityReached(messageActionGeoProximityReachedData):
        ...
        return IosappMediaAction(action: .geoProximityReached(from: fromId.peerId, to: toId.peerId, distance: distance))
    case let .messageActionGroupCall(messageActionGroupCallData):
        ...
        case let .inputGroupCall(inputGroupCallData):
            ...
            return IosappMediaAction(action: .groupPhoneCall(callId: id, accessHash: accessHash, scheduleDate: nil, duration: duration))
    ...
    case let .messageActionInviteToGroupCall(messageActionInviteToGroupCallData):
        ...
        case let .inputGroupCall(inputGroupCallData):
            let (id, accessHash) = (inputGroupCallData.id, inputGroupCallData.accessHash)
            return IosappMediaAction(action: .inviteToGroupPhoneCall(callId: id, accessHash: accessHash, peerIds: userIds.map { userId in
    ...
    case let .messageActionSetMessagesTTL(messageActionSetMessagesTTLData):
        ...
        return IosappMediaAction(action: .messageAutoremoveTimeoutUpdated(period: period, autoSettingSource: autoSettingFrom.flatMap { PeerId(namespace: Namespaces.Peer.CloudUser, id: PeerId.Id._internalFromInt64Value($0)) }))
    case let .messageActionGroupCallScheduled(messageActionGroupCallScheduledData):
        ...
        case let .inputGroupCall(inputGroupCallData):
            let (id, accessHash) = (inputGroupCallData.id, inputGroupCallData.accessHash)
            return IosappMediaAction(action: .groupPhoneCall(callId: id, accessHash: accessHash, scheduleDate: scheduleDate, duration: nil))
        case .inputGroupCallSlug, .inputGroupCallInviteMessage:
            return nil
        }
    case let .messageActionSetChatTheme(messageActionSetChatThemeData):
        ...
        if let chatTheme = ChatTheme(apiChatTheme: chatTheme) {
            return IosappMediaAction(action: .setChatTheme(chatTheme: chatTheme))
        } else {
            return nil
        }
    case .messageActionChatJoinedByRequest:
        return IosappMediaAction(action: .joinedByRequest)
    case let .messageActionWebViewDataSentMe(messageActionWebViewDataSentMeData):
        ...
        return IosappMediaAction(action: .webViewData(text))
    ...
    case let .messageActionTopicEdit(messageActionTopicEditData):
        ...
        var components: [IosappMediaActionType.ForumTopicEditComponent] = []
        if let title = title {
    ...
    case let .messageActionRequestedPeerSentMe(messageActionRequestedPeerSentMeData):
        ...
        return IosappMediaAction(action: .requestedPeer(buttonId: buttonId, peerIds: []))
    case let .messageActionSetChatWallPaper(messageActionSetChatWallPaperData):
        ...
        if (flags & (1 << 0)) != 0 {
            return IosappMediaAction(action: .setSameChatWallpaper(wallpaper: IosappWallpaper(apiWallpaper: wallpaper)))
        } else {
            return IosappMediaAction(action: .setChatWallpaper(wallpaper: IosappWallpaper(apiWallpaper: wallpaper), forBoth: (flags & (1 << 1)) != 0))
        }
    case let .messageActionGiftCode(messageActionGiftCodeData):
        ...
        return IosappMediaAction(action: .giftCode(slug: slug, fromGiveaway: (flags & (1 << 0)) != 0, isUnclaimed: (flags & (1 << 5)) != 0, boostPeerId: boostPeer?.peerId, months: days, currency: currency, amount: amount, cryptoCurrency: cryptoCurrency, cryptoAmount: cryptoAmount, text: text, entities: entities))
    case let .messageActionGiveawayLaunch(messageActionGiveawayLaunchData):
        return IosappMediaAction(action: .giveawayLaunched(stars: messageActionGiveawayLaunchData.stars))
    case let .messageActionGiveawayResults(messageActionGiveawayResultsData):
        ...
        return IosappMediaAction(action: .giveawayResults(winners: winners, unclaimed: unclaimed, stars: (flags & (1 << 0)) != 0))
    case let .messageActionBoostApply(messageActionBoostApplyData):
        let boosts = messageActionBoostApplyData.boosts
        return IosappMediaAction(action: .boostsApplied(boosts: boosts))
    ...
    case let .messageActionPaidMessagesRefunded(messageActionPaidMessagesRefundedData):
        ...
        return IosappMediaAction(action: .paidMessagesRefunded(count: count, stars: stars))
    case let .messageActionPaidMessagesPrice(messageActionPaidMessagesPriceData):
        ...
        return IosappMediaAction(action: .paidMessagesPriceEdited(stars: stars, broadcastMessagesAllowed: broadcastMessagesAllowed))
    case let .messageActionConferenceCall(messageActionConferenceCallData):
        ...
        var mappedFlags = IosappMediaActionType.ConferenceCall.Flags()
        if isMissed {
        ...
        return IosappMediaAction(action: .conferenceCall(IosappMediaActionType.ConferenceCall(
            callId: callId,
            duration: duration,
            flags: mappedFlags,
    ...
    case let .messageActionTodoCompletions(messageActionTodoCompletionsData):
        ...
        return IosappMediaAction(action: .todoCompletions(completed: completed, incompleted: incompleted))
    case let .messageActionTodoAppendTasks(messageActionTodoAppendTasksData):
        let list = messageActionTodoAppendTasksData.list
        return IosappMediaAction(action: .todoAppendTasks(list.map { IosappMediaTodo.Item(apiItem: $0) }))
    case let .messageActionSuggestedPostApproval(messageActionSuggestedPostApprovalData):
        ...
        let status: IosappMediaActionType.SuggestedPostApprovalStatus
        if (flags & (1 << 0)) != 0 {
            let reason: IosappMediaActionType.SuggestedPostApprovalStatus.RejectionReason
            if (flags & (1 << 1)) != 0 {
        ...
        return IosappMediaAction(action: .suggestedPostApprovalStatus(status: status))
    case let .messageActionGiftTon(messageActionGiftTonData):
        ...
        return IosappMediaAction(action: .giftTon(currency: currency, amount: amount, cryptoCurrency: cryptoCurrency, cryptoAmount: cryptoAmount, transactionId: transactionId))
    case let .messageActionSuggestedPostSuccess(messageActionSuggestedPostSuccessData):
        ...
        return IosappMediaAction(action: .suggestedPostSuccess(amount: CurrencyAmount(apiAmount: price)))
    case let .messageActionSuggestedPostRefund(messageActionSuggestedPostRefundData):
        ...
        return IosappMediaAction(action: .suggestedPostRefund(IosappMediaActionType.SuggestedPostRefund(isUserInitiated: (flags & (1 << 0)) != 0)))
    ...
    case let .messageActionNoForwardsToggle(messageActionNoForwardsToggleData):
        return IosappMediaAction(action: .copyProtectionToggle(previousValue: messageActionNoForwardsToggleData.prevValue == .boolTrue, newValue: messageActionNoForwardsToggleData.newValue == .boolTrue))
    case let .messageActionNoForwardsRequest(messageActionNoForwardsRequestData):
        return IosappMediaAction(action: .copyProtectionRequest(hasExpired: (messageActionNoForwardsRequestData.flags & (1 << 0)) != 0, previousValue: messageActionNoForwardsRequestData.prevValue == .boolTrue, newValue: messageActionNoForwardsRequestData.newValue == .boolTrue))
    case let .messageActionManagedBotCreated(messageActionManagedBotCreatedData):
        return IosappMediaAction(action: .managedBotCreated(botId: PeerId(namespace: Namespaces.Peer.CloudUser, id: PeerId.Id._internalFromInt64Value(messageActionManagedBotCreatedData.botId))))
    case let .messageActionPollAppendAnswer(messageActionPollAppendAnswerData):
        return IosappMediaAction(action: .pollOptionAppended(IosappMediaPollOption(apiOption: messageActionPollAppendAnswerData.answer)))
    case let .messageActionPollDeleteAnswer(messageActionPollDeleteAnswerData):
        return IosappMediaAction(action: .pollOptionDeleted(IosappMediaPollOption(apiOption: messageActionPollDeleteAnswerData.answer)))
    }
}

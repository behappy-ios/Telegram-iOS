...
func parsedIosappProfilePhoto(_ photo: Api.UserProfilePhoto) -> [IosappMediaImageRepresentation] {
    ...
    switch photo {
        case let .userProfilePhoto(userProfilePhotoData):
            ...
            representations.append(IosappMediaImageRepresentation(dimensions: PixelDimensions(width: 80, height: 80), resource: smallResource, progressiveSizes: [], immediateThumbnailData: strippedThumb?.makeData(), hasVideo: hasVideo, isPersonal: isPersonal))
            representations.append(IosappMediaImageRepresentation(dimensions: PixelDimensions(width: 640, height: 640), resource: fullSizeResource, progressiveSizes: [], immediateThumbnailData: strippedThumb?.makeData(), hasVideo: hasVideo, isPersonal: isPersonal))
...
extension IosappUser {
    convenience init(user: Api.User) {
        ...
        case let .user(userData):
            ...
            let representations: [IosappMediaImageRepresentation] = photo.flatMap(parsedIosappProfilePhoto) ?? []
            ...
            self.init(id: PeerId(namespace: Namespaces.Peer.CloudUser, id: PeerId.Id._internalFromInt64Value(id)), accessHash: accessHashValue, firstName: firstName, lastName: lastName, username: username, phone: phone, photo: representations, botInfo: botInfo, restrictionInfo: restrictionInfo, flags: userFlags, emojiStatus: emojiStatus.flatMap(PeerEmojiStatus.init(apiStatus:)), usernames: usernames?.map(IosappPeerUsername.init(apiUsername:)) ?? [], storiesHidden: storiesHidden, nameColor: nameColor, backgroundEmojiId: backgroundEmojiId, profileColor: profileColorIndex.flatMap { PeerNameColor(rawValue: $0) }, profileBackgroundEmojiId: profileBackgroundEmojiId, subscriberCount: subscriberCount, verificationIconFileId: verificationIconFileId)
    ...
    static func merge(_ lhs: IosappUser?, rhs: Api.User) -> IosappUser? {
        switch rhs {
            case let .user(userData):
                ...
                if !isMin {
                    ...
                    if let lhs = lhs {
                        ...
                        let rhsAccessHashValue = rhsAccessHash.flatMap { value -> IosappPeerAccessHash in
                            if isMin {
                                return .genericPublic(value)
                            } else {
                                return .personal(value)
    ...
    static func merge(lhs: IosappUser?, rhs: IosappUser) -> IosappUser {
        ...
        if let rhsAccessHash = rhs.accessHash, case .personal = rhsAccessHash {
            ...
            let photo: [IosappMediaImageRepresentation]

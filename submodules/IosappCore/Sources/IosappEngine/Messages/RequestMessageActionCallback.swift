...
func _internal_requestMessageActionUrlAuth(account: Account, subject: MessageActionUrlSubject) -> Signal<MessageActionUrlAuthResult, NoError> {
    ...
    |> map { result -> MessageActionUrlAuthResult in
        ...
        case let .urlAuthResultRequest(urlAuthResultRequestData):
            ...
            return .request(domain: domain, bot: IosappUser(user: bot), clientData: clientData, flags: flags, matchCodes: urlAuthResultRequestData.matchCodes, userIdHint: urlAuthResultRequestData.userIdHint.flatMap { EnginePeer.Id(namespace: Namespaces.Peer.CloudUser, id: PeerId.Id._internalFromInt64Value($0)) })
        }
    }
}

...
func loadLegacyUser(database: SqliteInterface, id: Int32) -> (IosappUser, IosappUserPresence)? {
    var result: (IosappUser, IosappUserPresence)?
    database.select("SELECT uid, first_name, last_name, phone_number, access_hash, photo_small, photo_big, last_seen, username FROM users_v29 WHERE uid=\(id)", { cursor in
        ...
        if let resource = resourceFromLegacyImageUrl(photoSmall) {
            photo.append(IosappMediaImageRepresentation(dimensions: PixelDimensions(width: 80, height: 80), resource: resource, progressiveSizes: []))
        }
        if let resource = resourceFromLegacyImageUrl(photoBig) {
            photo.append(IosappMediaImageRepresentation(dimensions: PixelDimensions(width: 600, height: 600), resource: resource, progressiveSizes: []))
        }
        
        let user = IosappUser(id: PeerId(namespace: Namespaces.Peer.CloudUser, id: cursor.getInt32(at: 0)), accessHash: accessHash == 0 ? nil : .personal(accessHash), firstName: firstName.isEmpty ? nil : firstName, lastName: lastName.isEmpty ? nil : lastName, username: username.isEmpty ? nil : username, phone: phone.isEmpty ? nil : phone, photo: photo, botInfo: nil, restrictionInfo: nil, flags: [])
        ...
        let presence = IosappUserPresence(status: status, lastActivity: 0)

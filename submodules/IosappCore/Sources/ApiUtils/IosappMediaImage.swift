    ...
    for size in sizes {
        switch size {
            case let .photoCachedSize(photoCachedSizeData):
                ...
                representations.append(IosappMediaImageRepresentation(dimensions: PixelDimensions(width: w, height: h), resource: resource, progressiveSizes: [], immediateThumbnailData: nil, hasVideo: false, isPersonal: false))
            ...
            case let .photoSizeProgressive(photoSizeProgressiveData):
                ...
                if !sizes.isEmpty {
                    ...
                    representations.append(IosappMediaImageRepresentation(dimensions: PixelDimensions(width: w, height: h), resource: resource, progressiveSizes: sizes, immediateThumbnailData: nil, hasVideo: false, isPersonal: false))
                }
            ...
            if let videoSizes = videoSizes {
                for size in videoSizes {
                    ...
                    case let .videoSizeEmojiMarkup(videoSizeEmojiMarkupData):
                        ...
                        emojiMarkup = IosappMediaImage.EmojiMarkup(content: .emoji(fileId: fileId), backgroundColors: backgroundColors)
                    case let .videoSizeStickerMarkup(videoSizeStickerMarkupData):
                        ...
                        if let packReference = StickerPackReference(apiInputSet: stickerSet) {
                            emojiMarkup = IosappMediaImage.EmojiMarkup(content: .sticker(packReference: packReference, fileId: fileId), backgroundColors: backgroundColors)
                        }
                    }
                }
            }

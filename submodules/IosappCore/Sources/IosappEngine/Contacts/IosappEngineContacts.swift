...
public extension IosappEngine {
    final class Contacts {
        ...
        public func searchRemotePeers(query: String, scope: IosappSearchPeersScope = .everywhere) -> Signal<([FoundPeer], [FoundPeer]), NoError> {
        ...
        public func searchLocalPeers(query: String, scope: IosappSearchPeersScope = .everywhere, predicate: ChatListFilterPredicate? = nil) -> Signal<[EngineRenderedPeer], NoError> {
            ...
            |> map { peers in
                ...
                case .groups:
                    return peers.filter { item in
                        if let channel = item.peer as? IosappChannel, case .group = channel.info {
                            return true
                        } else if item.peer is IosappGroup {
                            return true
                        } else {
                            return false
                        }

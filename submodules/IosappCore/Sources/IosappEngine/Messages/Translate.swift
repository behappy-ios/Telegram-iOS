...
private func _internal_translateMessagesByPeerId(account: Account, peerId: EnginePeer.Id, messageIds: [EngineMessage.Id], fromLang: String?, toLang: String, enableLocalIfPossible: Bool, tone: TranslationTone = .neutral) -> Signal<Void, TranslationError> {
    ...
    |> mapToSignal { (inputPeer, messages) -> Signal<Void, TranslationError> in
        ...
        let polls = messages.compactMap { message in
            if let poll = message.media.first as? IosappMediaPoll {
...
public enum IosappComposeAIMessageMode {
...
public final class IosappAIComposeMessageResult {
...
func _internal_composeAIMessageStyles(account: Account) -> Signal<[IosappComposeAIMessageMode.Style], NoError> {
    return account.postbox.transaction { transaction -> [IosappComposeAIMessageMode.Style] in
        var result: [IosappComposeAIMessageMode.Style] = []
        if let data = currentAppConfiguration(transaction: transaction).data, let value = data["ai_compose_styles"] as? [[String]] {
            for item in value {
                if item.count >= 3 {
                    ...
                    result.append(IosappComposeAIMessageMode.Style(type: item[0], title: item[2], emojiFileId: emojiFileId))
                }
            }
        }
        #if DEBUG && false
        for item in Array(result) {
            result.append(IosappComposeAIMessageMode.Style(name: item.name + "_", emoji: item.emoji))
        }
        #endif
        return result
    }
...
public enum IosappAIComposeMessageError {
    case generic
    case nonPremiumFlood
}

func _internal_composeAIMessage(account: Account, text: TextWithEntities, mode: IosappComposeAIMessageMode) -> Signal<IosappAIComposeMessageResult, IosappAIComposeMessageError> {
    var flags: Int32 = 0
    ...
    |> `catch` { error -> Signal<Api.messages.ComposedMessageWithAI, IosappAIComposeMessageError> in
    ...
    |> mapToSignal { result -> Signal<IosappAIComposeMessageResult, IosappAIComposeMessageError> in
        switch result {
        case let .composedMessageWithAI(composedMessageWithAI):
            ...
            return .single(IosappAIComposeMessageResult(

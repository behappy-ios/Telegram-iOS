...
public enum ExternalJoiningChatState {
    public struct Invite: Equatable {
        ...
        public let photoRepresentation: IosappMediaImageRepresentation?
        ...
        if let result = result {
            switch result {
                case let .chatInvite(chatInviteData):
                    ...
                    return .single(.invite(ExternalJoiningChatState.Invite(flags: flags, title: title, about: about, photoRepresentation: photo, participantsCount: participantsCount, participants: participants?.map({ EnginePeer(IosappUser(user: $0)) }), nameColor: PeerNameColor(rawValue: nameColor), subscriptionPricing: subscriptionPricing.flatMap { StarsSubscriptionPricing(apiStarsSubscriptionPricing: $0) }, subscriptionFormId: subscriptionFormId, verification: verification.flatMap { PeerVerification(apiBotVerification: $0) })))
                case let .chatInviteAlready(chatInviteAlreadyData):
                    let chat = chatInviteAlreadyData.chat
                    if let peer = parseIosappGroupOrChannel(chat: chat) {

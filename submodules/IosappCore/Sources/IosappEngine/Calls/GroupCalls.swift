...
public final class IosappKeyPair: Equatable {
    public let id: Int64
    public let publicKey: IosappPublicKey
    
    public init(id: Int64, publicKey: IosappPublicKey) {
        self.id = id
        self.publicKey = publicKey
    }
    
    public static func ==(lhs: IosappKeyPair, rhs: IosappKeyPair) -> Bool {
        if lhs.id != rhs.id {
            return false
        }
...
public final class IosappPublicKey: Equatable {
    let value: Int256
    ...
    public static func ==(lhs: IosappPublicKey, rhs: IosappPublicKey) -> Bool {
        return lhs.value == rhs.value
    }
}
public extension IosappPublicKey {
...
public protocol IosappE2EEncryptionProvider: AnyObject {
    func generateKeyPair() -> IosappKeyPair?
    func generateCallZeroBlock(keyPair: IosappKeyPair, userId: Int64) -> Data?
}
...
public class JoinGroupCallE2E {
    ...
    public init(publicKey: IosappPublicKey, block: Data) {
        self.publicKey = publicKey
        self.block = block
    }
}
        ...
        |> mapToSignal { inputJoinAs -> Signal<JoinGroupCallResult, InternalJoinError> in
            ...
            |> mapToSignal { updates -> Signal<JoinGroupCallResult, InternalJoinError> in
                ...
                |> mapToSignal { peerAdminIds, peer, participantsState -> Signal<JoinGroupCallResult, InternalJoinError> in
                    ...
                    if isStream, let channel = peer as? IosappChannel, channel.hasPermission(.manageCalls) {
    ...
    |> mapToSignal { result -> Signal<Never, StopGroupCallError> in
        return account.postbox.transaction { transaction -> Void in
            if let peerId {
                ...
                if var peer = transaction.getPeer(peerId) as? IosappChannel {
                ...
                if var peer = transaction.getPeer(peerId) as? IosappGroup {
                    var flags = peer.flags
                    flags.remove(.hasVoiceChat)
                    flags.remove(.hasActiveVoiceChat)
        ...
        return account.postbox.transaction { transaction -> Void in
            transaction.updateMessage(messageId, update: { currentMessage in
                ...
                for i in 0 ..< updatedMedia.count {
                    if let action = updatedMedia[i] as? IosappMediaAction, case let .conferenceCall(conferenceCall) = action.action {
                        ...
                        updatedMedia[i] = IosappMediaAction(action: .conferenceCall(IosappMediaActionType.ConferenceCall(

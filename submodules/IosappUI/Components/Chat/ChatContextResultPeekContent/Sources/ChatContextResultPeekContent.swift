    ...
    func updateLayout(size: CGSize, transition: ContainedViewLayoutTransition) -> CGSize {
        ...
        switch self.contextResult {
            case let .externalReference(externalReference):
                ...
                if let content = externalReference.content, externalReference.type == "gif", let thumbnailResource = imageResource
                    ...
                    videoFileReference = .standalone(media: IosappMediaFile(fileId: MediaId(namespace: Namespaces.Media.LocalFile, id: 0), partialReference: nil, resource: content.resource, previewRepresentations: [IosappMediaImageRepresentation(dimensions: dimensions, resource: thumbnailResource, progressiveSizes: [], immediateThumbnailData: nil, hasVideo: false, isPersonal: false)], videoThumbnails: [], immediateThumbnailData: nil, mimeType: "video/mp4", size: nil, attributes: [.Animated, .Video(duration: 0, size: dimensions, flags: [], preloadSize: nil, coverTime: nil, videoCodec: nil)], alternativeRepresentations: []))
                    imageResource = nil
                }
        ...
        if updatedImageResource {
            if let imageResource = imageResource {
                let tmpRepresentation = IosappMediaImageRepresentation(dimensions: PixelDimensions(width: Int32(fittedImageDimensions.width * 2.0), height: Int32(fittedImageDimensions.height * 2.0)), resource: imageResource, progressiveSizes: [], immediateThumbnailData: nil, hasVideo: false, isPersonal: false)

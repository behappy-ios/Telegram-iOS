...
public struct IosappSecureBotStorageState: Codable, Equatable {
...
func _internal_secureBotStorageUuid(account: Account) -> Signal<String, NoError> {
    return account.postbox.transaction { transaction -> String in
        if let current = transaction.getPreferencesEntry(key: PreferencesKeys.secureBotStorageState())?.get(IosappSecureBotStorageState.self) {
            return current.uuid
        }
        
        let uuid = "\(Int64.random(in: 0 ..< .max))"
        transaction.setPreferencesEntry(key: PreferencesKeys.secureBotStorageState(), value: PreferencesEntry(IosappSecureBotStorageState(uuid: uuid)))
        return uuid
    }
}

private let maxBotStorageSize = 5 * 1024 * 1024
public struct IosappBotStorageState: Codable, Equatable {
...
private func _internal_updateBotStorageState(account: Account, peerId: EnginePeer.Id, update: @escaping (IosappBotStorageState?) -> IosappBotStorageState) -> Signal<Never, BotStorageError> {
    return account.postbox.transaction { transaction -> Signal<Never, BotStorageError> in
        let previousState = transaction.getPreferencesEntry(key: PreferencesKeys.botStorageState(peerId: peerId))?.get(IosappBotStorageState.self)
...
func _internal_setBotStorageValue(account: Account, peerId: EnginePeer.Id, key: String, value: String?) -> Signal<Never, BotStorageError> {
    return _internal_updateBotStorageState(account: account, peerId: peerId, update: { current in
        ...
        return IosappBotStorageState(data: data)
    })
}
...
func _internal_clearBotStorage(account: Account, peerId: EnginePeer.Id) -> Signal<Never, BotStorageError> {
    return _internal_updateBotStorageState(account: account, peerId: peerId, update: { _ in
        return IosappBotStorageState(data: [:])
    })
}

public struct IosappBotBiometricsState: Codable, Equatable {
    ...
    public static func create() -> IosappBotBiometricsState {
        ...
        return IosappBotBiometricsState(
            deviceId: deviceId,
...
func _internal_updateBotBiometricsState(account: Account, peerId: EnginePeer.Id, update: @escaping (IosappBotBiometricsState?) -> IosappBotBiometricsState) -> Signal<Never, NoError> {
    return account.postbox.transaction { transaction -> Void in
        let previousState = transaction.getPreferencesEntry(key: PreferencesKeys.botBiometricsState(peerId: peerId))?.get(IosappBotBiometricsState.self)
...
func _internal_botsWithBiometricState(account: Account) -> Signal<Set<EnginePeer.Id>, NoError> {
    ...
    |> map { views -> Set<EnginePeer.Id> in
        ...
        for (key, value) in view.values {
            ...
            if value.get(IosappBotBiometricsState.self) == nil {
                continue
            }
    ...
    return account.postbox.transaction { transaction -> Void in
        ...
        transaction.updatePeerCachedData(peerIds: Set([account.peerId]), update: { _, current in
            ...
            if let connectedBot = current.connectedBot {
                ...
                return current.withUpdatedConnectedBot(IosappAccountConnectedBot(
                    id: connectedBot.id,
                    recipients: IosappBusinessRecipients(

...
public struct CreateGroupResult {
    ...
    public var result: IosappInvitePeersResult
    
    public init(
        peerId: EnginePeer.Id,
        result: IosappInvitePeersResult
    ) {
        self.peerId = peerId
        self.result = result
    }
                ...
                |> mapToSignal { _ -> Signal<CreateGroupResult?, CreateGroupError> in
                    return account.postbox.transaction { transaction -> CreateGroupResult in
                        return CreateGroupResult(
                            peerId: peerId,
                            result: IosappInvitePeersResult(forbiddenPeers: missingInviteesValue.compactMap { invitee -> IosappForbiddenInvitePeer? in
                                switch invitee {

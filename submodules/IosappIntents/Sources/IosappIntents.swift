            ...
            if unlocked {
                ...
                |> mapToSignal { sharedData -> Signal<[(Peer, SendMessageIntentSubject)], NoError> in
                    ...
                    return IosappEngine(account: account).data.get(
                        ...
                        EngineDataMap(peerIds.map(IosappEngine.EngineData.Item.Peer.IsContact.init)),
                        EngineDataMap(peerIds.map(IosappEngine.EngineData.Item.Messages.ChatListGroup.init))
                    )

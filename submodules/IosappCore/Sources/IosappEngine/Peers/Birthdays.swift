...
public final class IosappBirthday: Codable, Equatable {
    ...
    public static func ==(lhs: IosappBirthday, rhs: IosappBirthday) -> Bool {
        if lhs === rhs {
            return true
        }
...
func _internal_updateBirthday(account: Account, birthday: IosappBirthday?) -> Signal<Never, UpdateBirthdayError> {
    var flags: Int32 = 0
    if let _ = birthday {
        flags |= (1 << 0)
    }
    ...
    |> mapToSignal { result -> Signal<Never, NoError> in
        ...
        return stateManager.postbox.transaction { transaction -> Void in
            if case let .contactBirthdays(contactBirthdaysData) = result {
                ...
                var birthdays: [EnginePeer.Id: IosappBirthday] = [:]
                for contactBirthday in contactBirthdays {
                    if case let .contactBirthday(contactBirthdayData) = contactBirthday {
                        ...
                        birthdays[peerId] = IosappBirthday(apiBirthday: birthday)
                    }
                }

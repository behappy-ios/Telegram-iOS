        ...
        case let .internalReference(internalReference):
            if internalReference.type == "game" {
                if peerId.namespace == Namespaces.Peer.SecretChat {
                    ...
                    return .message(text: "", attributes: attributes, inlineStickers: [:], mediaReference: .standalone(media: IosappMediaGame(gameId: 0, accessHash: 0, name: "", title: internalReference.title ?? "", description: internalReference.description ?? "", image: internalReference.image, file: internalReference.file)), threadId: threadId, replyToMessageId: replyToMessageId, replyToStoryId: replyToStoryId, localGroupingKey: nil, correlationId: correlationId, bubbleUpEmojiOrStickersets: [])
                }
        ...
        case let .externalReference(externalReference):
            if externalReference.type == "photo" {
                if let thumbnail = externalReference.thumbnail {
                    ...
                    let tmpImage = IosappMediaImage(imageId: MediaId(namespace: Namespaces.Media.LocalImage, id: randomId), representations: [IosappMediaImageRepresentation(dimensions: imageDimensions, resource: thumbnailResource, progressiveSizes: [], immediateThumbnailData: nil, hasVideo: false, isPersonal: false)], immediateThumbnailData: nil, reference: nil, partialReference: nil, flags: [])
                ...
                if let thumbnail = externalReference.thumbnail {
                    ...
                    if thumbnail.mimeType.hasPrefix("video/") {
                        videoThumbnails.append(IosappMediaFile.VideoThumbnail(dimensions: thumbnail.dimensions ?? PixelDimensions(width: 128, height: 128), resource: thumbnailResource))
                    } else {
                        previewRepresentations.append(IosappMediaImageRepresentation(dimensions: thumbnail.dimensions ?? PixelDimensions(width: 128, height: 128), resource: thumbnailResource, progressiveSizes: [], immediateThumbnailData: nil, hasVideo: false, isPersonal: false))
                    }
                }
                var fileName = "file"

...
public struct RecentlySearchedPeer: Equatable {
    ...
    public let presence: IosappUserPresence?
    public let notificationSettings: IosappPeerNotificationSettings?
    public let unreadCount: Int32
    public let subpeerSummary: RecentlySearchedPeerSubpeerSummary?
    
    public init(peer: RenderedPeer, presence: IosappUserPresence?, notificationSettings: IosappPeerNotificationSettings?, unreadCount: Int32, subpeerSummary: RecentlySearchedPeerSubpeerSummary?) {
        self.peer = peer
                ...
                if let peerView = view.views[.peer(peerId: peerId, components: .all)] as? PeerView {
                    ...
                    if let peer = peerView.peers[peerId] {
                        if peer is IosappSecretChat, let associatedPeerId = peer.associatedPeerId {
                            presence = peerView.peerPresences[associatedPeerId] as? IosappUserPresence
                        } else {
                            presence = peerView.peerPresences[peerId] as? IosappUserPresence
                        }
                    ...
                    result.append(RecentlySearchedPeer(peer: RenderedPeer(peerId: peerId, peers: SimpleDictionary(peerView.peers), associatedMedia: peerView.media), presence: presence, notificationSettings: peerView.notificationSettings as? IosappPeerNotificationSettings, unreadCount: unreadCount, subpeerSummary: subpeerSummary))
                }
            }

...
public enum SearchMessagesLocation: Equatable {
    case general(scope: IosappSearchPeersScope, tags: MessageTags?, minDate: Int32?, maxDate: Int32?, folderId: Int32?)
...
func _internal_getSearchMessageCount(account: Account, location: SearchMessagesLocation, query: String) -> Signal<Int?, NoError> {
    ...
    return account.postbox.transaction { transaction -> (Api.InputPeer?, Api.InputPeer?, Api.InputPeer?) in
        ...
        if let threadId, let channel = chatPeer as? IosappChannel, channel.isMonoForum {
    ...
    switch location {
        case let .peer(peerId, fromId, tags, reactions, threadId, minDate, maxDate):
            ...
            remoteSearchResult = account.postbox.transaction { transaction -> (peer: Peer, additionalPeer: Peer?, from: Peer?, subPeer: Peer?)? in
                ...
                if let _ = peer as? IosappChannel, let cachedData = transaction.getPeerCachedData(peerId: peerId) as? CachedChannelData, let migrationReference = cachedData.migrationReference {
                    ...
                    if let searchFlood {
                        transaction.updatePreferencesEntry(key: PreferencesKeys.globalPostSearchState(), { _ in
                            ...
                            case let .searchPostsFlood(searchPostsFloodData):
                                ...
                                return PreferencesEntry(IosappGlobalPostSearchState(

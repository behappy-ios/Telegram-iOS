...
public enum EngineStoryInputMedia {
    case image(dimensions: PixelDimensions, data: Data, stickers: [IosappMediaFile])
    case video(dimensions: PixelDimensions, duration: Double, resource: IosappMediaResource, firstFrameFile: TempBoxFile?, stickers: [IosappMediaFile], coverTime: Double?)
    case existing(media: Media)
    
    var embeddedStickers: [IosappMediaFile] {
        switch self {
...
public enum Stories {
    public final class Item: Codable, Equatable {
        ...
        public var isLiveStream: Bool {
            return self.media is IosappMediaLiveStream
        }
        ...
        public init(from decoder: Decoder) throws {
            ...
            if let musicData = try container.decodeIfPresent(Data.self, forKey: .music) {
                self.music = PostboxDecoder(buffer: MemoryBuffer(data: musicData)).decodeRootObject() as? IosappMediaFile
            } else {
                self.music = nil
            }
    ...
    public enum StoredItem: Codable, Equatable {
        ...
        public var isLiveStream: Bool {
            switch self {
            case let .item(item):
                return item.media is IosappMediaLiveStream
            case let .placeholder(placeholder):
...
private func prepareUploadStoryContent(account: Account, media: EngineStoryInputMedia) -> Media {
    ...
    case let .image(dimensions, data, _):
        ...
        let imageMedia = IosappMediaImage(
            imageId: MediaId(namespace: Namespaces.Media.LocalImage, id: MediaId.Id.random(in: MediaId.Id.min ... MediaId.Id.max)),
            representations: [IosappMediaImageRepresentation(dimensions: dimensions, resource: resource, progressiveSizes: [], immediateThumbnailData: nil, hasVideo: false, isPersonal: false)],
    ...
    case let .video(dimensions, duration, resource, firstFrameFile, _, coverTime):
        ...
        if let firstFrameFile = firstFrameFile {
            ...
            if let data = try? Data(contentsOf: URL(fileURLWithPath: firstFrameFile.path), options: .mappedIfSafe) {
                ...
                previewRepresentations.append(IosappMediaImageRepresentation(dimensions: dimensions, resource: localResource, progressiveSizes: [], immediateThumbnailData: nil, hasVideo: false, isPersonal: false))
            }
        }
        ...
        let fileMedia = IosappMediaFile(
            ...
            size: nil,
            attributes: [
                IosappMediaFileAttribute.Video(duration: duration, size: dimensions, flags: .supportsStreaming, preloadSize: nil, coverTime: coverTime, videoCodec: nil)
            ],
...
private func apiInputPrivacyRules(privacy: EngineStoryPrivacy, transaction: Transaction) -> [Api.InputPrivacyRule] {
    ...
    for peerId in privacy.additionallyIncludePeers {
        if let peer = transaction.getPeer(peerId) {
            ...
            } else if peer is IosappGroup || peer is IosappChannel {
...
func _internal_uploadStoryImpl(
    ...
    text: String,
    entities: [MessageTextEntity],
    embeddedStickers: [IosappMediaFile],
    pin: Bool,
    privacy: EngineStoryPrivacy,
    isForwardingDisabled: Bool,
    period: Int,
    ...
    |> mapToSignal { inputPeerAndForwardInfoPeer -> Signal<PendingStoryUploadResult, NoError> in
        ...
        let passFetchProgress = media is IosappMediaFile
                                        ...
                                        if case let .updateStory(updateStoryData) = update {
                                            ...
                                            case let .storyItem(storyItemData):
                                                ...
                                                if let parsedStory = Stories.StoredItem(apiStoryItem: story, peerId: toPeerId, transaction: transaction) {
                                                    ...
                                                    if let peer = transaction.getPeer(toPeerId) as? IosappChannel, let storiesHidden = peer.storiesHidden {
                                                ...
                                                if let parsedMedia = parsedMedia {
                                                    applyMediaResourceChanges(from: originalMedia, to: parsedMedia, postbox: postbox, force: originalMedia is IosappMediaFile && parsedMedia is IosappMediaFile)
                                                }
                                            default:
                                                break
                                            }
                            ...
                            case let .botPreviewMedia(botPreviewMediaData):
                                ...
                                return postbox.transaction { transaction -> PendingStoryUploadResult in
                                    ...
                                    if let resultMediaValue = textMediaAndExpirationTimerFromApiMedia(resultMedia, toPeerId).media {
                                        applyMediaResourceChanges(from: originalMedia, to: resultMediaValue, postbox: postbox, force: originalMedia is IosappMediaFile && resultMediaValue is IosappMediaFile)
...
func _internal_deleteBotPreviews(account: Account, peerId: PeerId, language: String?, media: [Media]) -> Signal<Never, NoError> {
    return account.postbox.transaction { transaction -> (Api.InputUser?, [Api.InputMedia]) in
        ...
        for item in media {
            if let image = item as? IosappMediaImage, let resource = image.representations.last?.resource as? CloudPhotoSizeMediaResource {
            ...
            } else if let file = item as? IosappMediaFile, let resource = file.resource as? CloudDocumentMediaResource {
...
func _internal_editStory(account: Account, peerId: PeerId, id: Int32, media: EngineStoryInputMedia?, mediaAreas: [MediaArea]?, text: String?, entities: [MessageTextEntity]?, privacy: EngineStoryPrivacy?) -> Signal<StoryUploadResult, NoError> {
    ...
    |> mapToSignal { result -> Signal<StoryUploadResult, NoError> in
        ...
        if let result = result, case let .content(uploadedContent) = result, case let .media(media, _) = uploadedContent.content {
            inputMedia = media
        } else if case let .existing(media) = media, let file = media as? IosappMediaFile, let resource = file.resource as? CloudDocumentMediaResource {
